...
void Enemy::setWalKPace(int pace){
    this->WalkPace=pace;
}
void Enemy::setRunPace(int pace){
    this->RunPace=pace;
}
...
bool Enemy::eye_contact(int Cx,int Cy){//像素
    int nx=this->map_x/this->dijk->copymap.widthpix,ny=this->map_y/this->dijk->copymap.heightpix;
    int tx=Cx/this->dijk->copymap.widthpix,ty=Cy/this->dijk->copymap.heightpix;
...
*/
    ...
    if(this->toward==1){
        for(int t=-1;t<=1;t++)
        {
            ...
            for(int k=1;k<=Length;k++){
                if(nx+t<0||ny+k<0||nx+t>=this->dijk->copymap.cols||ny+k>=this->dijk->copymap.rows)break;
                if(this->dijk->copymap.Martix[ny+k][nx+t]!=0&&this->dijk->copymap.Martix[ny+k][nx+t]!=1)break;
                if(nx+t==tx&&ny+k==ty){
                    return true;
                }
    ...
    if(this->toward==2){//左
        for(int t=-1;t<=1;t++)
        {
            ...
            for(int k=1;k<=Length;k++){
                if(nx-k<0||ny+t<0||nx-k>=this->dijk->copymap.cols||ny+t>=this->dijk->copymap.rows)break;
                if(this->dijk->copymap.Martix[ny+t][nx-k]!=0&&this->dijk->copymap.Martix[ny+t][nx-k]!=1)break;
                if(nx-k==tx&&ny+t==ty){
                    return true;
                }
...
bool Enemy::map_check(int x, int y) {
    ...
    int width = this->dijk->copymap.cols*this->dijk->copymap.widthpix, height = this->dijk->copymap.rows * this->dijk->copymap.heightpix;
    int nx=x/this->dijk->copymap.widthpix,ny=y/this->dijk->copymap.heightpix;
    if (x<0||x+this->dijk->copymap.widthpix>width||y<0||y+this->dijk->copymap.heightpix>height)
        return false;
    ...
    if(index==0){
        ...
        if(index==1){
          ...
          //  qDebug() << "函数运行时间：" << elapsed << "毫秒";
            ...
            int tid=this->y1/this->dijk->copymap.heightpix*this->dijk->copymap.cols+this->x1/this->dijk->copymap.widthpix;
         ...
         //   qDebug()<<"tid="<<tid<<",tid_x="<<tid%this->dijk->copymap.cols<<",y="<<tid/this->dijk->copymap.cols;
            ...
            if(this->state!=3){
                ...
                if(nx==this->x1/this->dijk->copymap.widthpix&&ny==this->y1/this->dijk->copymap.heightpix){
                    this->state=2;
                }
            }
            this->working=false;
            ...
            if(index==2){
                ...
                stack<int>q;
                int nid=ny*this->dijk->copymap.cols+nx;
                   int tid=this->y2/this->dijk->copymap.heightpix*this->dijk->copymap.cols+this->x2/this->dijk->copymap.widthpix;
                while(tid!=nid&&path.PRE[tid]!=-1){
                    if(tid==nid)break;
                    q.push(tid);
                    tid=path.PRE[tid];
                ...
                if(this->state!=3){
                    ...
                    if(nx==this->x2/this->dijk->copymap.widthpix&&ny==this->y2/this->dijk->copymap.heightpix){
                        this->state=1;
                    }
                }
                this->working=false;
