...
class QtHello;
class QtBattle;
QtBattle::QtBattle(QWidget* parent, Defences* HelloCharacter,Enemy* HelloEnemy,std::string BattelBackPath, std::string littleimgPath) {
	//初始化变量
	turn_num = 0;
    if(!HelloCharacter){
        qDebug()<<"Oh no!When you new the QtBattle,you don't send the HelloCharacter";
    }
	animation = nullptr;
	bloodImg2 = nullptr;
   ...
   //qDebug()<<QUrl::fromLocalFile(QString("music\\battlemusic.mp3"));
    ...
    QString menustr=menustr1.arg("HP:").arg((to_string(this->blood1_now)+"/"+to_string(this->blood1_max)).data()).arg("\nMP:").arg((to_string(this->magic1_now)+"/"+to_string(this->magic1_max)).data());
	...
	smallImageLabel->setPixmap(smallImage.scaled(360 * PercentWidth, 200 * PercentHeight));//16：9
...
void QtBattle::button1_clicked()
{
    ...
    //敌人防御力当做0
    double realhurt=this->defence->getAttack()+this->button1->getSkillHurt()-this->enemy->getDefence();//担心没破防
    ...
    QMovie* attack = new QMovie("gifs\\Chongyue\\attack.gif");
    attack->setSpeed(300);
	...
	//当动画播放完毕后，删除动画
    connect(attack, &QMovie::frameChanged, [this, attack,realhurt] {
...
void QtBattle::button2_clicked()
{
	...
	//根据当前技能对敌人伤害的不同确定skilleffct
    double realhurt=this->defence->getAttack()+this->button2->getSkillHurt()-this->enemy->getDefence();//担心没破防
    //根据当前技能对敌人伤害的不同确定skilleffct
    double hurtrate = double(max(0.0,realhurt)) / double(this->blood2_max);
    ...
    QMovie* skill1 = new QMovie("gifs\\Chongyue\\Skill1.gif");
    skill1->setSpeed(300);
	...
	//当动画播放完毕后，删除动画
    connect(skill1, &QMovie::frameChanged, [this, skill1,realhurt] {
...
void QtBattle::button3_clicked() {
	...
	//根据当前技能对敌人伤害的不同确定skilleffct
    double realhurt=this->defence->getAttack()+this->button3->getSkillHurt()-this->enemy->getDefence();//担心没破防
    //根据当前技能对敌人伤害的不同确定skilleffct
    double hurtrate = double(max(0.0,realhurt)) / double(this->blood2_max);
    ...
    QMovie* skill3 = new QMovie("gifs\\Chongyue\\Skiil3.gif");
    skill3->setSpeed(300);
	...
	//当动画播放完毕后，删除动画
    connect(skill3, &QMovie::frameChanged, [this, skill3,realhurt] {
...
void QtBattle::button4_clicked() {
    ...
    double realhurt=this->defence->getAttack()+this->button4->getSkillHurt()-this->enemy->getDefence();//担心没破防
    //根据当前技能对敌人伤害的不同确定skilleffct
    double hurtrate = double(max(0.0,realhurt)) / double(this->blood2_max);
    ...
    QMovie* skill2b = new QMovie("gifs\\Chongyue\\Skill-2-Begin.gif");
    QMovie* skill2e = new QMovie("gifs\\Chongyue\\Skill-2-End.gif");
	...
	//当动画播放完毕后，删除动画
    connect(skill2b, &QMovie::frameChanged, [this, skill2b, skill2e,realhurt] {
		...
		if (skill2b->currentFrameNumber() == skill2b->frameCount() - 1) {
            ...
            connect(skill2e, &QMovie::frameChanged, [this, skill2e,realhurt] {
...
void QtBattle::EnemyLogic(){
    ...
    double realhurt=this->enemy->getAttack()+enemey_hurt-this->defence->getDefence();//担心没破防
    //根据当前技能对敌人伤害的不同确定skilleffct
    ...
    QMovie* skillenemey = new QMovie("gifs\\Bob\\bobattack.gif");
    skillenemey->setSpeed(300);
    this->characterLabel2->setMovie(skillenemey);



    //当动画播放完毕后，删除动画
    connect(skillenemey, &QMovie::frameChanged, [this, skillenemey,realhurt] {

        //判断是否是最后一帧
        if (skillenemey->currentFrameNumber() == skillenemey->frameCount() - 1) {
...
void QtBattle::turnBegin() {
    ...
    if(this->blood1_now*1.0<=this->blood1_max*0.3){
        if(this->music_id!=2)
        {
            this->music_id=2;
            this->player->setSource(QUrl::fromLocalFile(QString("music\\manlowhp.mp3")));
        this->player->setLoops(-1);
        this->player->play();
        }
        }
    if(this->blood2_now*1.0<=this->blood2_max*0.3){
        if(this->music_id!=3)

        {
        this->music_id=3;
        this->player->setSource(QUrl::fromLocalFile(QString("music\\enemylowhp.mp3")));
        this->player->setLoops(-1);
        this->player->play();
        }
        }
    ...
    if (if_my_turn) {
        ...
        if(button1->getSkillCost()>this->magic1_now)button1->setEnabled(false);
        if(button2->getSkillCost()>this->magic1_now)button2->setEnabled(false);
        if(button3->getSkillCost()>this->magic1_now)button3->setEnabled(false);
        if(button4->getSkillCost()>this->magic1_now)button4->setEnabled(false);
        qDebug()<<"blood1_now="<<this->blood1_now<<",blood1_max="<<this->blood1_max;
        qDebug()<<"magic1_now="<<this->magic1_now<<",magic1_max="<<this->magic1_max;
        qDebug()<<"button1 hurt="<<button1->getSkillHurt()<<",button1 cost="<<button1->getSkillCost();
        qDebug()<<"button2 hurt="<<button2->getSkillHurt()<<",button2 cost="<<button2->getSkillCost();
        qDebug()<<"button3 hurt="<<button3->getSkillHurt()<<",button3 cost="<<button3->getSkillCost();
        qDebug()<<"button4 hurt="<<button4->getSkillHurt()<<",button4 cost="<<button4->getSkillCost();
	}
	//若是敌人回合，敌人采取行动
	else {
...
void ShowLabel::wrap(bool b, int last_count) {
	...
	for (int i = last_count; i < discription.count(); i++)
	{
		...
		if (i > 1) {
            ...
            /*else if ((cha == QString("害")) && this->discription.at(i - 1) == QString("伤")) {//由于是否显著，已经不需要他了
...
void ShowLabel::setdefence(int D){
    this->defence=D;
}
void ShowLabel::setattack(int A){
    this->attack=A;
}
